...
(function () {
  const { model, folder, templates, trackerImport } = window.APPTool;
  ...
  function renderOverview() {
    const rows = state.tracker.strands.map((s) => {
      ...
      const flagged = list.filter((iv) => ['behind', 'soon'].includes(model.slipCheck(iv, TODAY).flag)).length;
    ...
    <p class="muted small">Status counts use the committee's four categories. "Flagged" counts interventions behind their planned stage or due to move stage within 14 days.</p>`;
  }
  ...
  function renderStrand(n) {
    ...
    const cards = list.map((iv) => {
      ...
      return `<article class="card flag-${check.flag}" data-id="${esc(iv.id)}">
        <header>
          <h3><span class="id">${esc(iv.id)}</span> ${esc(iv.name)}</h3>
          <div class="pills">${statusPill(iv.status)}${check.flag === 'bau' ? '' : ' ' + flagPill(check)}</div>
        </header>
        <p class="desc">${esc(iv.description)}</p>
        <div class="facts">
          <div><h4>Where it should be</h4>${periodsText(iv)}</div>
          <div><h4>Stage check</h4><p>${esc(check.text)}</p></div>
  ...
  function render() {
    ...
    const tabs = [['strand', 'Strands'], ['audit', 'Template audit'], ['import', 'Import tracker']];
    $('#tabs').innerHTML = tabs.map(([k, l]) => `<button role="tab" aria-selected="${state.tab === k}" data-tab="${k}">${l}</button>`).join('');
    const body = state.tab === 'audit' ? renderAuditTab() : state.tab === 'import' ? renderImportTab() : renderStrandTab();
